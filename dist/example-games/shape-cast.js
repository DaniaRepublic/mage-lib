...
export class ShapeCastGame extends GameBaseClientDebug {
    ...
    async setup(domGameRoot) {
        ...
        world.createCollider(RAPIER.ColliderDesc.cuboid(5, .1, 5), floorBody);
        ...
        scene.add(cubeMesh);
        const cubeBody = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().lockRotations());
        cubeBody.setTranslation({ x: 0, y: 2, z: 0 }, true);
        this.cubeBody = cubeBody;
        world.createCollider(RAPIER.ColliderDesc.cuboid(.5, .5, .5)
            .setMass(1), cubeBody);
    }
    jumpImpulse = 8;
    ...
    physicsLoopLogic(deltaTime) {
        ...
        if (space && this.currJumpCoolDown === 0) {
            ...
            const intersecting = this.physics.world.intersectionWithShape(checkPosition, rotation, collider.shape, null, null, collider, this.cubeBody);
